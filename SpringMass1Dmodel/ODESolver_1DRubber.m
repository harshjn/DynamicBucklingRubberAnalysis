function [X,V,F] = ODESolver_1DRubber(x0,y0,Vx0,Vy0,L,N,dt,kL,kT,compressionFactor,gamma)
...
%     th = thetaRight(n)/2 + thetaLeft(n)/2;
    ...
    FyMat(n) = FyMat(n) + kT*t_dist*sin(theta(n)) - (1e-3/N)*g ; 
...
Vx1 = Vx0 + dt*FxMat/(1e-3/N);
Vy1 = Vy0 + dt*FyMat/(1e-3/N);
%BoundaryConditions
