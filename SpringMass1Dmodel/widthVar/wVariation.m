...
dt = 1e-5;
T=0.07;
wMat = 1e-2:1e-2:10e-2;
tMat = 0: dt: T;
i = 23; % Select Speed index
for j = 1:10
    clearvars -except dt wMat j tMat T speedMat i speedMat lengthsMatMin lengthsMatMax
    speed=speedMat(i);
    
    N=10;
    L=0.1;
    w = wMat(j); t = 1e-3; G = 3e6;
    kL=G*w*t/L;
    I = w*t^3/12;
    kT = G*I/L^3;
    gamma = 0.1;
    ...
    %     [X,V,F] = ODESolver_1DRubber(xMat(n,tInd-1),yMat(n,tInd-1),VxMat(n,tInd-1),VyMat(n,tInd-1));
        [X,V,F] = ODESolver_1DRubber(xMat(:,tInd-1),yMat(:,tInd-1),VxMat(:,tInd-1)...
            ,VyMat(:,tInd-1),L,N,dt,w,kL,kT,compressionFactor,gamma);
